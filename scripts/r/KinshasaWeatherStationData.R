...
# Save to PNG
ggsave("G:/Research/Kinshasa_EarthObserve/precipitation_comparison.png", precip_comp_plot,
...
# Create the plot object for temp comparisons
temp_comp_plot <- ggplot(temp_compare, aes(x = month, y = avg_temp, color = station)) +
  geom_line(size = 1.2) +
  geom_point(size = 2) +
  labs(
    title = "Monthly Average Temperature – All Stations",
...
# Save to PNG
ggsave("G:/Research/Kinshasa_EarthObserve/temperature_comparison.png", temp_comp_plot,
       width = 10, height = 6, dpi = 300)
	   
	   
# Create the plot object for precip comparisons
precip_comp_plot <- ggplot(precip_compare, aes(x = month, y = total_precip, color = station)) +
  geom_line(size = 1.2) +
  geom_point(size = 2) +
  labs(
    title = "Monthly Total Precipitation – All Stations",
...
# Save NDJILI grid.arrange plot to PNG
png("G:/Research/Kinshasa_EarthObserve/ndjili_combined_plot.png", width = 10, height = 8, units = "in", res = 300)
grid.arrange(temp_plot, precip_plot, ncol = 1)
dev.off()

# BINZA
png("G:/Research/Kinshasa_EarthObserve/binza_combined_plot.png", width = 10, height = 8, units = "in", res = 300)
grid.arrange(temp_plot_binza, precip_plot_binza, ncol = 1)
dev.off()

# MAYA MAYA
png("G:/Research/Kinshasa_EarthObserve/maya_combined_plot.png", width = 10, height = 8, units = "in", res = 300)
grid.arrange(temp_plot_maya, precip_plot_maya, ncol = 1)
dev.off()

# N'DOLO
png("G:/Research/Kinshasa_EarthObserve/ndolo_combined_plot.png", width = 10, height = 8, units = "in", res = 300)
grid.arrange(temp_plot_ndolo, precip_plot_ndolo, ncol = 1)
dev.off()
